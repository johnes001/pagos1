...
switch ($metodo) {
	...
	case "listar":
		...
		$conex->cerrar();
		require_once($template_ruta.'calendario_listar.phtml');
		...
		if($_REQUEST['m2'] == "ielts")
		{
			...
			if(isset($_REQUEST['calendar']))
			{
				...
				require_once($template_ruta.'vista_previa_calendario_ielts.phtml');				
...
else if($_REQUEST['m2'] == "ielts-internacional")
		{
			...
			if(isset($_REQUEST['calendar']))
			{
				...
				require_once($template_ruta.'vista_previa_calendario_ielts_internacional.phtml');				
		...
		else if ($_REQUEST['m2'] == "general")
		{
			//VISTA PREVIA GENERAL ***********************************************************
				if(isset($_REQUEST['calendar']))
			{	
				require_once($template_ruta.'vista_previa_calendario_general.phtml');	
			}
		}
	...
	case "insertar":
		...
		if(isset($_REQUEST['guardar'])) {
			...
			//creando la URL
			$url_calendario= new url_calendario($_REQUEST['NOMBRE'], " ");
			$nombre_pasarela=$url_calendario->limpiarUrl();
			$url_final="https://multilingua.edu.co/pagos/index.php?calendar=";
			$url_final.=$nombre_pasarela;
			$url_final.="&m=listar";
			$url_final.="&c=calendario";
			$url_final.="&m2=";
			$url_final.=$_REQUEST['GRUPO'];
			...
			"mail_creador" => $_REQUEST['PROPIETARIO'],
			"activo" => $_REQUEST['ACTIVO'],
			"grupo" => $_REQUEST['GRUPO'],
			"url" => $url_final);
				
			if($conex->insertar("CALENDARIOS",$datos,$conexion))
			{
				$conex->cerrar();
				$titulo="Calendario";
				$mensaje= "Se ha creado el calendario correctamente";
			...
			else
			{
				...
				$mensaje= "El Calendario que esta creando con el nombre <b>".$_REQUEST['NOMBRE']."</b> ya existe, cambie el Nombre o ponga al final un n&uacute;mero para identificarlo";
			...
			$conex->cerrar();
			require_once($template_ruta.'calendario_crear.phtml');
		}
		break;
	...
	case "editar":
		...
		$resultado=$conex->listar($conexion, 'CALENDARIOS', $orden);
			...
			if($conex->actualizar("CALENDARIOS", 'ID', $id, $datos))
			{
				$conex->cerrar();
			...
			else
			{
				$conexion->cerrar();
				$titulo="Verifique las siguientes recomendaciones";
				$mensaje= "El Calendario que est&aacute; actualizando con el nombre <b>".$_REQUEST['NOMBRE']."</b> contiene un Nombre duplicado &oacute; revise los campos para corregir la informaci&oacute;n, si desea cambiar este Calendario por uno similar, cambie el Nombre o ponga al final un n&uacute;mero para identificarlo";
		...
		else if(isset($_REQUEST['editar_form']))
		{
			$propietarios=$conex->listar_columnas('DISTINCT', 'USUARIOS', 'MAIL');
			$conex->cerrar();
			require_once($template_ruta.'calendario_editar_form.phtml');
		}
		else
		{
			require_once($template_ruta.'calendario_editar.phtml');
		}
